 ...
 * Argument: replacement - the new character to replace the old
 * Argument: str         - the string array
 * Argument: maxLength   - the size of the array holding the string
 */
void replaceChar(char old, char replacement, char* str, int maxLength) {
    int i = 0;
    for (i = 0; i < maxLength && str[i] != '\0'; i++)
        if (str[i] == old)
            str[i] = replacement;
...
int main(void) {
    ...
    printf("%s\n", copy);
    safeStringCopy(buffer, copy, BUFFER_SIZE);
    printf("%s\n", copy);
